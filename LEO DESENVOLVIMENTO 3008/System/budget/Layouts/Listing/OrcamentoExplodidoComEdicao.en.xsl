				...
				<body>
					<table align="center" border="0">
							...
							<tr>
								<td align="middle">
									...
									<table width="630" border="0">
										<tbody>
											<tr>
												<td style="FONT-WEIGHT: bold; FONT-SIZE: 12px; COLOR: #808080; PADDING-TOP: 20px; FONT-FAMILY: Arial" width="400">Date 
												...
												<td style="FONT-WEIGHT: bold; FONT-SIZE: 12px; COLOR: #808080; PADDING-TOP: 20px; FONT-FAMILY: Arial" align="right">Budget 
												</td>
											</tr>
										</tbody>
									</table>
											...
											<xsl:for-each select="CATEGORIES/CATEGORY">
												<thead>
													...
													<tr id="columns" bgcolor="#e9e9e9">
														...
														<th id="material" width="7%">
															<b>Raw Material 
															</b>
														</th>
														<th id="reference" width="23%">
															<b>Reference 
															</b>
														</th>
														<th id="description" width="20%">
															<b>Description 
															</b>
														</th>
														<th id="size" width="15%">
														...
														<th id="salesprice" align="right" width="8%">
															<b>Table Price 
															</b>
														</th>
									...
									<table style="BORDER-RIGHT: #cecece 1px solid; BORDER-TOP: #cecece 1px solid; BORDER-LEFT: #cecece 1px solid; BORDER-BOTTOM: #cecece 1px solid" cellspacing="0" cellpadding="0" width="100%" border="0">
										<tbody>
											...
											<tr align="middle" bgcolor="#e9e9e9">
												...
												<th style="PADDING-RIGHT: 5px; PADDING-LEFT: 5px; PADDING-BOTTOM: 5px; PADDING-TOP: 5px" width="25%">
													<b>
														<font style="BACKGROUND-COLOR: #e9e9e9">Total value 
														</font>
													</b>
												</th>
											</tr>
									...
									<tbody>
										<tr bgcolor="#e9e9e9">
											<td style="PADDING-LEFT: 5px; FONT-WEIGHT: bold; FONT-SIZE: 10px; WIDTH: 80px; COLOR: #818181; LINE-HEIGHT: 20px; FONT-FAMILY: Arial">
												<b>
													<span style="COLOR: #535353">Table Price: 
													</span>
												</b>
											</td>
											<td>
											...
											<td style="PADDING-RIGHT: 5px; FONT-SIZE: 10px; FONT-FAMILY: Arial" align="right" width="65">
												<b>
													...
													<xsl:if xmlns:xsl="http://www.w3.org/1999/XSL/Transform" test="count(TOTALPRICES/@TABLE) &gt; 0">
														<xsl:variable select="translate(TOTALPRICES/@TABLE, ',', '.')" name="valor">
														</xsl:variable>
													<xsl:value-of select="format-number($valor, '###.##0,00', 'formato')" />
													</xsl:if>
													<xsl:if xmlns:xsl="http://www.w3.org/1999/XSL/Transform" test="count(TOTALPRICES/@TABLE) = 0"> 0,00 
													</xsl:if>
												</b>
											</td>
										</tr>
						...
						<tfoot id="FOOTER">
							...
							<tr>
								<td align="middle">©Promob Studio. All rignts reserved. 
								</td>
							</tr>
						</tfoot>
  ...
  <tbody>
    ...
    <td align="right" style="padding-right:1px;">
      <xsl:if test="count(PRICE) &gt; 0">
        ...
        <div align="right">
          <a href="javascript:void(0);">
            <xsl:attribute name="Title"> Preço Tabela: <xsl:value-of select="format-number($valorPreco, '###.##0,00', 'formato')" /></xsl:attribute>
            <xsl:attribute name="onClick">
              <xsl:if test="count(REFERENCES/PRICE) &gt; 0">	
								    external.InvokeVSAComMensagem('${messages_budget:msg_text_changePrice}','${messages_budget:msg_title_changePrice}','FormApresentacaoListagem','AlterarPreco','<xsl:value-of select="REFERENCES/PRICE/@REFERENCE" />;<xsl:value-of select="@QUANTITY" />;<xsl:value-of select="PRICE/@TABLE" />;<xsl:value-of select="PRICE/@UNIT" />;<xsl:value-of select="PRICE/@TOTALCOMPONENTS" />;<xsl:value-of select="@UNIT" />;<xsl:value-of select="@IDPRICEPROVIDER" />;<xsl:value-of select="@IDPRICESOURCE" />'); 
							    </xsl:if>
              <xsl:if test="count(REFERENCES/PRICE) = 0">	
								    external.InvokeVSAComMensagem('${messages_budget:msg_text_changePrice}','${messages_budget:msg_title_changePrice}','FormApresentacaoListagem','AlterarPreco','<xsl:value-of select="@REFERENCE" />;<xsl:value-of select="@QUANTITY" />;<xsl:value-of select="PRICE/@TABLE" />;<xsl:value-of select="PRICE/@UNIT" />;<xsl:value-of select="PRICE/@TOTALCOMPONENTS" />;<xsl:value-of select="@UNIT" />;<xsl:value-of select="@IDPRICEPROVIDER" />;<xsl:value-of select="@IDPRICESOURCE" />'); 
							    </xsl:if>
            </xsl:attribute>
            <xsl:if test="$nivel = 0">
              <b>
      ...
      <xsl:if test="count(PRICE) = 0">
        ...
        <div align="right">
          <a href="javascript:void(0);">
            <xsl:attribute name="Title">Preço Tabela: 0,00 </xsl:attribute>
            <xsl:attribute name="onClick">
              <xsl:if test="count(REFERENCES/PRICE) &gt; 0">	
								    external.InvokeVSAComMensagem('${messages_budget:msg_text_changePrice}','${messages_budget:msg_title_changePrice}','FormApresentacaoListagem','AlterarPreco','<xsl:value-of select="REFERENCES/PRICE/@REFERENCE" />;<xsl:value-of select="@QUANTITY" />;0;0;<xsl:value-of select="PRICE/@TOTALCOMPONENTS" />;<xsl:value-of select="@UNIT" />;<xsl:value-of select="@IDPRICEPROVIDER" />;<xsl:value-of select="@IDPRICESOURCE" />'); 
							    </xsl:if>
              <xsl:if test="count(REFERENCES/PRICE) = 0">
                                    external.InvokeVSAComMensagem('${messages_budget:msg_text_changePrice}','${messages_budget:msg_title_changePrice}','FormApresentacaoListagem','AlterarPreco','<xsl:value-of select="@REFERENCE" />;<xsl:value-of select="@QUANTITY" />;0;0;<xsl:value-of select="PRICE/@TOTALCOMPONENTS" />;<xsl:value-of select="@UNIT" />;<xsl:value-of select="@IDPRICEPROVIDER" />;<xsl:value-of select="@IDPRICESOURCE" />'); 
                                </xsl:if>
            </xsl:attribute>
            <xsl:if test="$nivel = 0">
              <b>
