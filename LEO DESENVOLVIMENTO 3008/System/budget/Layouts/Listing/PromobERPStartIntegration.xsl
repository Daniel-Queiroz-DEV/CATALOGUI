...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="LISTING">
    <XML_BUILDER TIPO="PROMOB_ERP_START">
      <ABOUTPROMOB>
        ...
        <ITENS_PEDIDO>
          <xsl:for-each select="AMBIENTS/AMBIENT/CATEGORIES/CATEGORY/ITEMS/ITEM">
            <xsl:element name="ITEM">
              ...
              <xsl:call-template name="BUSCAR_ID_ERP" />
              </xsl:element>
  ...
  <xsl:template name ="BUSCAR_ID_ERP">
    <xsl:if test="count(ADDITIONAL_INFORMATIONS_BUILDER)  &gt; 0">
      <xsl:if test="count(ADDITIONAL_INFORMATIONS_BUILDER/@ID_ERP)  &gt; 0">
        <xsl:attribute name="ID_ERP">
          <xsl:value-of select="ADDITIONAL_INFORMATIONS_BUILDER/@ID_ERP" />
        </xsl:attribute>
      </xsl:if>
    </xsl:if>
  </xsl:template>
    ...
    <xsl:if test="count(ADDITIONAL_INFORMATIONS_BUILDER)  &gt; 0">
      <xsl:if test="count(ADDITIONAL_INFORMATIONS_BUILDER/@ID_ERP)  &gt; 0">
        <xsl:element name="UNIQUE_ID">
          ...
          <xsl:attribute name="AMBIENTGUID">
            <xsl:value-of select="parent::node()/parent::node()/parent::node()/parent::node()/parent::node()/@GUID" />
          </xsl:attribute>
          <xsl:attribute name="AMBIENTNAME">
            <xsl:value-of select="parent::node()/parent::node()/parent::node()/parent::node()/parent::node()/@DESCRIPTION" />
          </xsl:attribute>
          <xsl:attribute name="PROJECTNAME">
            <xsl:value-of select="parent::node()/parent::node()/parent::node()/parent::node()/parent::node()/@DESCRIPTION" />
          </xsl:attribute>
  ...
  <xsl:template name="GERAR_NODO_MAQUINAS">
    <xsl:element name="MAQUINAS">
      <xsl:if test="count(METADATAS) &gt; 0">
        <xsl:if test="count(METADATAS/METADATA) &gt; 0">
          <!--<xsl:if test="METADATAS/METADATA/@ID = 'NESTINGENTITYDATA'">
            <xsl:if test="count(METADATAS/METADATA/METADATA.VALUE)  &gt; 0 ">
              <xsl:if test="count(METADATAS/METADATA/METADATA.VALUE/NESTINGENTITYDATA)  &gt; 0">
                <xsl:if test="count(METADATAS/METADATA/METADATA.VALUE/NESTINGENTITYDATA/MAQUINAS)  &gt; 0">
                  <xsl:for-each select="METADATAS/METADATA/METADATA.VALUE/NESTINGENTITYDATA/MAQUINAS/MAQUINA">
                      ...
                      <xsl:element name="SETUPS">
                        <xsl:for-each select="SETUPS/SETUP">
                          <xsl:element name="SETUP">
                            ...
                            <xsl:if test="count(PROGRAMAS)  &gt; 0">
                              <xsl:if test="count(PROGRAMAS/PROGRAMA)  &gt; 0">
